...
public class MessageThreadUtil<T> implements ThreadUtil<T> {
    ...
    public class C02941 implements ThreadUtil.MainThreadCallback<T> {
        ...
        private Runnable mMainThreadRunnable = new Runnable() { // from class: androidx.recyclerview.widget.MessageThreadUtil.1.1
            ...
            public void run() {
                while (true) {
                    SyncQueueItem next = C02941.this.mQueue.next();
                    if (next == null) {
                        return;
                    }
                    int i = next.what;
                    if (i == 1) {
                        C02941.this.val$callback.updateItemCount(next.arg1, next.arg2);
                    } else if (i == 2) {
                        C02941.this.val$callback.addTile(next.arg1, (TileList.Tile) next.data);
                    } else if (i != 3) {
                        Log.e("ThreadUtil", "Unsupported message, what=" + next.what);
                    } else {
                        C02941.this.val$callback.removeTile(next.arg1, next.arg2);
                    }
                }
            }
        };

        public C02941(ThreadUtil.MainThreadCallback mainThreadCallback) {
    ...
    public class C02952 implements ThreadUtil.BackgroundCallback<T> {
        ...
        private Runnable mBackgroundRunnable = new Runnable() { // from class: androidx.recyclerview.widget.MessageThreadUtil.2.1
            ...
            public void run() {
                while (true) {
                    SyncQueueItem next = C02952.this.mQueue.next();
                    if (next == null) {
                    ...
                    int i = next.what;
                    if (i == 1) {
                        C02952.this.mQueue.removeMessages(1);
                        C02952.this.val$callback.refresh(next.arg1);
                    } else if (i == 2) {
                        C02952.this.mQueue.removeMessages(2);
                        C02952.this.mQueue.removeMessages(3);
                        C02952.this.val$callback.updateRange(next.arg1, next.arg2, next.arg3, next.arg4, next.arg5);
                    } else if (i == 3) {
                        C02952.this.val$callback.loadTile(next.arg1, next.arg2);
                    } else if (i != 4) {
                        Log.e("ThreadUtil", "Unsupported message, what=" + next.what);
                    } else {
                        C02952.this.val$callback.recycleTile((TileList.Tile) next.data);
                    }
                }
            }
        };

        public C02952(ThreadUtil.BackgroundCallback backgroundCallback) {
    ...
    public ThreadUtil.BackgroundCallback<T> getBackgroundProxy(ThreadUtil.BackgroundCallback<T> backgroundCallback) {
        return new C02952(backgroundCallback);
    }

    @Override // androidx.recyclerview.widget.ThreadUtil
    public ThreadUtil.MainThreadCallback<T> getMainThreadProxy(ThreadUtil.MainThreadCallback<T> mainThreadCallback) {
        return new C02941(mainThreadCallback);
    }
}
